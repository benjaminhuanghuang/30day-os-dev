...
void sheet_refreshmap(struct SHTCTL *ctl, int vx0, int vy0, int vx1, int vy1, int h0)
{
	...
	for (h = h0; h <= ctl->top; h++)
	{
		...
		if (sht->col_inv == -1)
		{
			...
			{
				/* High-speed version dedicated to no transparent color (1-byte type) */
				for (by = by0; by < by1; by++)
				{
					vy = sht->vy0 + by;
...
void sheet_refreshsub(struct SHTCTL *ctl, int vx0, int vy0, int vx1, int vy1, int h0, int h1)
{
	...
	struct SHEET *sht;
	/* clip when the refresh range extends beyond the screen */
	if (vx0 < 0) { vx0 = 0; }
	if (vy0 < 0) { vy0 = 0; }
	...
	for (h = h0; h <= h1; h++) {
		...
		buf = sht->buf;
		sid = sht - ctl->sheets0;
		/* Use vx0 to vy1 to calculate bx0 to by1 */
		bx0 = vx0 - sht->vx0;
		by0 = vy0 - sht->vy0;
		bx1 = vx1 - sht->vx0;
		by1 = vy1 - sht->vy0;
		...
		if ((sht->vx0 & 3) == 0) {
			/* 4 byte */
			i  = (bx0 + 3) / 4; /* bx0 divided by 4 (rounded up to the nearest whole number)  */
			i1 =  bx1      / 4; /* bx1 divided by 4 (rounded down to the nearest whole number)  */
			i1 = i1 - i;
			sid4 = sid | sid << 8 | sid << 16 | sid << 24;
			for (by = by0; by < by1; by++) {
				vy = sht->vy0 + by;
				for (bx = bx0; bx < bx1 && (bx & 3) != 0; bx++) {	/* 前面被4除多余的部分 */
					vx = sht->vx0 + bx;
				...
				for (bx += i1 * 4; bx < bx1; bx++) {				/* 后面被4除多余的部分*/
					vx = sht->vx0 + bx;
					if (map[vy * ctl->xsize + vx] == sid) {
						vram[vy * ctl->xsize + vx] = buf[by * sht->bxsize + bx];
					}
