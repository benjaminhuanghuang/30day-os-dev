	...
	for (;;)
	{
		...
		else
		{
			...
			if (256 < i && i <= 511)
			{
				...
				if (i < 0x80 + 256) { /* Convert keycode to character code */
					if (key_shift == 0) {
						s[0] = keytable0[i - 256];
					} else {
						s[0] = keytable1[i - 256];
				...
				if (i == 256 + 0xfa) {	/* The keyboard received the data successfully. */
					keycmd_wait = -1;
				}
				if (i == 256 + 0xfe) {	/* The keyboard did not receive the data safely. */
					wait_KBC_sendready();
					io_out8(PORT_KEYDAT, keycmd_wait);
				}
				/* draw the cursor */
